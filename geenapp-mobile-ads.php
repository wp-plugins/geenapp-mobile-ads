...
Plugin URI: http://www.geenapp.com/
Description: Geenapp publisher tool
Author: Geenapp
    ...
    jQuery(document).ready(function($) {
      $("#signin").click(function() {
        ...
        $.post(ajaxurl, data, function(response) {
          ...
          switch(response) {
            case 0:
              txt = '<?php echo __( 'Token incorect, please check for the correct token at www.geenapp.com', 'geenapp'); ?>';
              $("#signin_messege").text(txt);
                break;
            case 1:
              txt = '<?php echo __( 'You just started to earn money with geenapp', 'geenapp'); ?>';
              $("#geenapp_signin").html(txt);
                break;
            default:
              txt = '<?php echo __( 'communication failure, please try again', 'geenapp'); ?>';
              $("#signin_messege").text(txt); 
          }
        });
      });
...
function geenapp_admin_init() {
  wp_register_style('geenapp_admin_css', plugins_url('geenapp-mobile-ads/geenapp-mobile-ads.css'));
...
function _geenapp_register() {
  ...
  echo '  <p>'.__('Please register at <a href="https://publisher.geenapp.com/register.php?a=wordpress">publisher.geenapp.com</a>. Thank You!', 'geenapp').'</p>';
  echo '</div>';
}
if(!function_exists("geenapp_menu")) {
  function geenapp_menu() {
    $page_title = 'Geenapp publisher'; 
    $menu_title = 'Geenapp';
...
?>
  <div class="tabbed-area cur-nav-fix">
    <div class="box-wrap">
      <div id="box-acount" class="tab-content"<?php if('reg' == get_option('geenapp_submit_tab')) echo ' style="z-index: 1;"'; ?>>
        <div class="geenapp_tab_contact">
<?php
    if ('' == get_option('geenapp_token')) {
      echo '<div id="geenapp_signin">
        <h1>'.__('geenapp token', 'geenapp').'</h1>
        <form method="post" action="admin-ajax.php">
          ...
          <p><button type="button" class="button button-primary" id="signin">'.__('sumit', 'geenapp').'</button></p>
        </form>
        <div style="top:250px;border:none" id="signin_messege"></div> 
      </div>';
    } else {
      echo '<div id="geenapp_signin">'.__('You just started to earn money with geenapp', 'geenapp').'</div>';
    }
?>
        </div>
        <ul class="tabs group">
          <li class="cur"><a href="#box-acount"><?php echo __('Account', 'geenapp'); ?></a></li>
          <li><a href="#box-five"><?php echo __('SmartBanner', 'geenapp'); ?></a></li>
        </ul>
      </div>
      <div id="box-five" class="tab-content"<?php if('fal' == get_option('geenapp_submit_tab')) echo ' style="z-index: 1;"'; ?>>
        <div class="geenapp_tab_contact">
...
?>
        ...
        <ul class="tabs group">
          <li><a href="#box-acount"><?php echo __('Account', 'geenapp'); ?></a></li>
          <li class="cur"><a href="#box-five"><?php echo __('SmartBanner', 'geenapp'); ?></a></li>
        </ul>
      </div>
    </div>
<?php
...
function geenapp_add_insert_footer() {
  ...
  <div onclick="document.getElementById(\'geenapp_faldon\').style.display=\'none\';" style="display: block; margin: auto; width: 320px; height: 50px;">
    <div style="position: relative; margin-bottom: -20px; float: right; right: -10px;"><img height="20" width="20" src="'.plugins_url().'/geenapp-mobile-ads/img/closer.png"></div>
    <img usemap="#admapfoot" id="geenapp_img" src="'.$adUrls['imageUrl'].'">
  </div>
</div>
<div id="geenapp_faldon_helper" style="height: 50px"></div>
<map name="admapfoot">
  <area shape="rect" coords="0,0,320,50" alt="geenapp" href="'.$adUrls['ckickUrl'].'">
</map>';
      ob_start();
      eval('?>' . $buffer);
    }
...
function geenapp_add_insert_head() {
  ...
  <div onclick="document.getElementById(\'geenapp_faldon\').style.display=\'none\'" style="display: block; margin: auto; width: 320px; height:50px;">
    <div style="position: relative; margin-bottom: -20px; float: right; right: -10px; top: 50px;"><img height="20" width="20" src="'.plugins_url().'/geenapp-mobile-ads/img/closer.png"></div>
...
function get_faldon_ad_url() {
  ...
  return array('ckickUrl' => $offer['url'], 'imageUrl' => $offer['ads']['ad320x50']);
}
function signin_callback() {
  ...
  echo $res->verified;
  if(($res->verified != -1) && ($res->verified != '')) {
