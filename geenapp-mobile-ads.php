...
function geenapp_ads_add_action_links($links ){
  $mylinks = array('<a href="' . admin_url( 'options-general.php?page=geenapp_post' ) . '">' .   __("Settings", "geenapp") .  '</a>', );
...
function geenapp_add_interstitial() {
  ...
  <div id="ad">
    <a href="#" onclick="document.getElementById(\'geenapp_fullscreen\').style.display=\'none\';" class="sb-close-pop">&#9733;</a>
...
function get_interstitial_ad_url() {
  ...
  $var['gee_source'] = $_SERVER['SERVER_NAME'];
...
function get_device() {
  if (stripos($_SERVER['HTTP_USER_AGENT'], 'iphone')) {
    ...
    $device = 'android';
	} elseif(stripos($_SERVER['HTTP_USER_AGENT'], 'windows phone') || stripos($_SERVER['HTTP_USER_AGENT'], 'windows mobile')) {
    $device = 'windows';
